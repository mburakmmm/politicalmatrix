...
import { normalizeVoteValue, resolvePartyRef, isGarbageLawId } from "../ai/enrichToolArgs";
...
import {
  ...
  applySoftCounterPressure,
  looksLikeHardReject,
} from "../sim/negotiationPressure";
import {
  computeMpRebellionSplit,
  readCoalitionStressForVoter,
} from "../sim/mpRebellion";
import { applyRegimeChange, isValidRegime, regimeAllowsElections, regimeAllowsParliament, getRegime } from "../sim/regime";
import { shareMinistriesForAlliance, reclaimMinistriesToHolder } from "../sim/ministries";
...
import { formatDeltas, lawFitsIdeology, resolveCatalogLawId } from "../sim/laws/catalog";
import {
  LAW_ID_COOLDOWN_MONTHS,
  suggestLawsForParty,
} from "../sim/lawSuggestions";
import { sanitizeRallyFocusTopic } from "../sim/rallyFocus";
import { canonicalizeToolName } from "../ai/toolAliases";
import {
  applyCabinetResignation,
  isCabinetLead,
  isGovernmentBlocMember,
} from "../sim/resignation";
...
export async function executePartyTool(
  ...
  const toolName = canonicalizeToolName(name);
  if (!regimeAllowsParliament(ctx.simulationId)) {
    ...
    if (!allowedWithoutParliament.has(toolName)) {
      const blocked = fail(
        "Meclis feshedildi; yalnızca rejim/güç araçları kullanılabilir."
      );
      if (!silent) trackRejection(ctx, toolName, blocked.message);
      return blocked;
    }
  }
  ...
  if (result.ok) {
    trackDecision(ctx, toolName, args, result.message);
  } else if (!silent) {
    trackRejection(ctx, toolName, result.message);
  }
  return result;
}
...
function proposeLaw(ctx: ExecCtx, args: Record<string, unknown>): ToolResult {
  ...
  const actor = getParty(ctx.actorPartyId)!;
  const garbageId =
    isGarbageLawId(rawId) ||
    rawId === "." ||
    rawId === "-" ||
    rawId === "?" ||
    rawId === "…" ||
    rawId === "..." ||
    /^(none|null|n\/a|na|unknown)$/i.test(rawId) ||
    /^\.+$/.test(rawId);

  let resolved = garbageId
    ? { law: null as ReturnType<typeof resolveCatalogLawId>["law"], note: undefined as string | undefined }
    : resolveCatalogLawId(rawId, actor.slug);
  let law = resolved.law;

  // Geçersiz / placeholder lawId → ideolojiye uygun katalog (her zaman dene)
  if (!law) {
    const pick = suggestLawsForParty(ctx.simulationId, actor.slug, 1)[0];
    if (pick && (garbageId || !rawId)) {
      law = pick;
      resolved = { law: pick, note: `boş/geçersiz lawId “${rawId}” → ${pick.id}` };
    } else if (pick && !garbageId) {
      // Bilinen ama çözülemeyen id — yine de öneri ver, otomatik seçme
    }
  }
  if (!law) {
    const alts = suggestLawsForParty(ctx.simulationId, actor.slug, 5)
      .map((l) => l.id)
      .join(", ");
    return fail(
      `Bilinmeyen lawId: ${rawId || "(boş)"}. Gerçek katalog id kullanın (policing_t2, military_t2, economy_t3…). Öneri: ${alts || "labor_t4"}`
    );
  }

  const fit = lawFitsIdeology(law, actor.slug);
  if (!fit.ok) {
    const alts = suggestLawsForParty(ctx.simulationId, actor.slug, 3)
      .map((l) => l.id)
      .join(", ");
    return fail(
  ...
  const recentSame = getDb()
    ...
    .get(
      ctx.simulationId,
      law.id,
      simEarly.month - LAW_ID_COOLDOWN_MONTHS
  ...
  if (recentSame) {
    const alts = suggestLawsForParty(ctx.simulationId, actor.slug, 3)
      ...
      .join(", ");
    const why =
      recentSame.status === "passed"
        ? "yakın zamanda kabul edildi"
        : recentSame.status === "rejected"
          ? "yakın zamanda reddedildi"
          : "zaten meclis/komisyonda";
    return fail(
      `“${law.title}” (${law.id}) ${why}. Başka id: ${alts || "—"}`
    );
  }
...
function voteOnBill(ctx: ExecCtx, args: Record<string, unknown>): ToolResult {
  ...
  const voteNorm = normalizeVoteValue(args.vote);
  ...
  // Konuşmayı yalnız konu dışı / boşsa hizala — ideoloji soft-coerce konuşmayı ezmesin
  ...
  // Milletvekili isyanı: lider oyu kayda geçer, sandalyeler bölünebilir
  const govLead = getParties(ctx.simulationId).find((p) => p.is_government);
  const stress = readCoalitionStressForVoter(
    ctx.simulationId,
    ctx.actorPartyId,
    govLead?.id ?? null
  );
  const votingAgainstGov =
    !!govLead &&
    (bill.proposer_id === govLead.id ||
      isGovernmentBlocMember(ctx.simulationId, bill.proposer_id));
  const split = computeMpRebellionSplit({
    seats: actor.seats,
    partyVote: effectiveVote,
    slug: actor.slug,
    law,
    isProposer: bill.proposer_id === ctx.actorPartyId,
    coalitionStress: stress,
    votingAgainstGovBill: votingAgainstGov && ctx.actorPartyId !== govLead?.id,
  });
  ...
  const rebelNote =
    split.rebelSeats > 0
      ? ` · ${split.rebelSeats} MV isyanı (${split.reason || "grup içi"})`
      : "";
  insertEvent(
    ctx.simulationId,
    "vote_cast",
    {
      ...
      seats: actor.seats,
      loyalSeats: split.loyalSeats,
      rebelSeats: split.rebelSeats,
      seatSplit: { yes: split.yes, no: split.no, abstain: split.abstain },
      message: `${actor.name}, “${bill.title}” için ${voteLabel} oyu verdi${coerceNote}${rebelNote} — “${cleanSpeech}”`,
    },
    sim.month
  );

  if (split.rebelSeats > 0) {
    insertEvent(
      ctx.simulationId,
      "mp_rebellion",
      {
        ...
        partyVote: effectiveVote,
        reason: split.reason,
        message: `${actor.name}: ${split.rebelSeats} milletvekili liderlik oyu (${voteLabel}) dışına çıktı — ${split.reason}.`,
      },
      sim.month
    );
  }
  ...
  // Koalisyon ortakları: hükümet yasasına Ret birikince stres → uyarı → kopma
  const coalitionStress = applyCoalitionVoteStress({
    simulationId: ctx.simulationId,
    voterPartyId: ctx.actorPartyId,
    proposerPartyId: bill.proposer_id,
    vote: effectiveVote,
    billTitle: bill.title,
    month: sim.month,
  });
  ...
  const stressNote = coalitionStress.ruptured
    ? " — koalisyon gerilimle koptu"
    : coalitionStress.warned
      ? ` — koalisyon stresi ${coalitionStress.stress.toFixed(0)}/100`
      : "";

  return ok(
    coerced
      ? `Oy kaydedildi: ${effectiveVote} (${coerceReason || "ideoloji düzeltmesi"})${stressNote}`
      : `Oy kaydedildi: ${effectiveVote}${stressNote}`,
    {
      billId: bill.id,
      vote: effectiveVote,
      speechText: cleanSpeech,
      coerced,
      coerceReason: coerced ? coerceReason : undefined,
      coalitionStress: coalitionStress.stress,
      coalitionRuptured: coalitionStress.ruptured,
    }
  );
}
...
/** Model kendi id/slug/adını hedef yazarsa düzelt; self/geçersizde null */
function resolveOtherPartyId(
  simulationId: string,
  actorPartyId: string,
  rawTarget: string
): string | null {
  return resolvePartyRef(simulationId, actorPartyId, rawTarget);
}
...
function negotiateCoalition(
  ...
  let targetPartyId = resolveOtherPartyId(
    ctx.simulationId,
    ctx.actorPartyId,
    String(args.targetPartyId || args.target || args.partyId || "")
  );
  ...
  // Hedef boşsa bakışa göre en iyi ortak
  if (!targetPartyId) {
    const others = getParties(ctx.simulationId).filter(
      (p) => p.id !== ctx.actorPartyId
    );
    let best: (typeof others)[0] | null = null;
    let bestScore = -999;
    for (const o of others) {
      const bias = attitudeVoteBias(ctx.actorPartyId, o.id);
      if (bias > bestScore) {
        bestScore = bias;
        best = o;
      }
    }
    if (best) targetPartyId = best.id;
  }
  ...
  if (!message) {
    const target = getParty(targetPartyId);
    const actor = getParty(ctx.actorPartyId);
    message = `${actor?.name || "Parti"}, ${target?.name || "ortak"} ile hükümet kurmak için koalisyon görüşmesi açıyor.`;
  }
  ...
  // accept:false siyasi red hakkı — sistem asla accept:true'ya yükseltmez
  ...
  if (negotiationId) {
    ...
    if (!accept && looksLikeHardReject(message)) {
      collapseNegotiation({
        simulationId: ctx.simulationId,
        negotiationId: neg.id,
        partyA: neg.from_party_id,
        partyB: neg.to_party_id,
        reason: `${actor.name} müzakereyi reddetti: ${message.slice(0, 80)}`,
        kind: "walk_away",
      });
      return fail(
        `Müzakere reddedildi — masa dağıldı. ${describeNegotiationPressure(neg.round)}`
      );
    }
    ...
    return ok(
      `Karşı teklif kaydı (r${newRound}). Kabul veya red sizin kararınız. ${describeNegotiationPressure(newRound)}`,
    ...
    if (existing) {
      ...
      // Karar turunda yanlış tool masayı çökertmesin — respondNegotiation beklenir
      if (isPastSoftPhase(existing.round)) {
        return fail(
          `Açık müzakere (r${existing.round}). respondNegotiation ile accept:true|false yanıtlayın; yeni soft negotiate açmayın.`
        );
      }
      if (existing.to_party_id === ctx.actorPartyId) {
        return fail(
          `Açık teklif size geldi (r${existing.round}). respondNegotiation kullanın (kabul veya red sizin).`
        );
      }
      return fail(
        `Açık müzakere sürüyor (r${existing.round}). Karşı tarafın respondNegotiation yanıtını bekleyin.`
      );
    }

    // Başka bir açık masa varken (farklı çift / karar turu) yeni masa açma
    const fresh = canOpenFreshNegotiation(ctx.simulationId, ctx.actorPartyId);
    if (!fresh.ok) {
      return fail(fresh.reason || "Açık müzakere varken yeni masa açılamaz.");
    }
...
function respondNegotiation(
  ...
  let negotiationId = String(args.negotiationId || "");
  let neg = negotiationId
    ? (getDb()
        .prepare(`SELECT * FROM negotiations WHERE id = ?`)
        .get(negotiationId) as
        | { from_party_id: string; to_party_id: string; id: string }
        | undefined)
    : undefined;

  // Model id kaçırırsa açık müzakereyi otomatik bağla
  if (!neg || neg.from_party_id === neg.to_party_id) {
    const open = getDb()
      .prepare(
        `SELECT * FROM negotiations
         WHERE simulation_id = ? AND status = 'open'
           AND to_party_id = ? AND from_party_id != to_party_id
         ORDER BY updated_month DESC LIMIT 1`
      )
      .get(ctx.simulationId, ctx.actorPartyId) as
      | { from_party_id: string; to_party_id: string; id: string }
      | undefined;
    if (open) {
      neg = open;
      negotiationId = open.id;
    }
  }
...
function breakAlliance(ctx: ExecCtx, args: Record<string, unknown>): ToolResult {
  ...
  if (!alliance) return fail("Aktif ittifak yok");

  const msg =
    reason ||
    `${actor.name}, ${other.name} ile ittifakı bozdu (koalisyon gerilimi).`;

  ruptureCoalitionAlliance({
    simulationId: ctx.simulationId,
    allianceId: alliance.id,
    initiatorId: ctx.actorPartyId,
    otherId: partyId,
    month: sim.month,
    reason: msg,
    automatic: false,
  ...
  if (gov) {
    insertEvent(
      ...
      "ministries_shared",
      {
        message: `İttifak bozuldu — bakanlıklar yeniden düzenlendi.`,
        partyName: gov.name,
...
function holdRally(ctx: ExecCtx, args: Record<string, unknown>): ToolResult {
  const actor = getParty(ctx.actorPartyId)!;
  const focusTopic = sanitizeRallyFocusTopic(args.focusTopic, actor.slug);
...
function issuePRStatement(
  ...
  let stance = String(args.stance || "deny").toLowerCase() as PRStance;
  const rawStatement = String(
    args.statementText || args.statement || ""
  ).trim();
  if (!["resign", "deny", "reform"].includes(stance) || !rawStatement) {
    return fail("stance (resign|deny|reform) ve statementText zorunlu");
  }
  const actor = getParty(ctx.actorPartyId)!;
  const sim = getSimulation(ctx.simulationId)!;
  const fallback =
    stance === "resign"
      ? `${actor.name} sorumluluğu üstlenerek istifa ediyor.`
      : stance === "deny"
        ? `${actor.name} suçlamaları reddediyor ve hukuki süreci bekliyor.`
        : `${actor.name} reform, şeffaflık ve hesap verebilirlik sözü veriyor.`;
  const statementText = sanitizePublicSpeech(rawStatement, fallback);
  ...
  if (corruption && !inBloc) {
    return fail(
      "Yolsuzluk skandalı PR'si yalnız iktidar veya koalisyon ortağı içindir. Muhalefet miting/gensoru kullanabilir."
    );
  }

  // İstifa: yalnız kabine bloğu — gerçek siyasi sonuç
  if (stance === "resign" && !inBloc) {
    return fail(
      "İstifa (resign) yalnız mühürlü iktidar veya koalisyon ortağı için. Muhalefet deny/reform veya gensoru kullanın."
    );
  }
  ...
  if (stance === "deny") {
    ...
    updateParty(actor.id, {
      poll_share: clampMetric(actor.poll_share - 2.5, 5, 70),
    });
    updateMetrics(ctx.simulationId, {
      freedom: clampMetric(metrics.freedom + 1),
      fear: clampMetric(metrics.fear - 1),
    });
  } else {
    updateParty(actor.id, {
  ...
  let resignNote = "";
  if (stance === "resign" && inBloc) {
    const outcome = applyCabinetResignation({
      simulationId: ctx.simulationId,
      partyId: ctx.actorPartyId,
      reason: statementText,
      month: sim.month,
    });
    if (!outcome.ok) {
      return fail(outcome.message);
    }
    resignNote = ` — ${outcome.message}`;
  } else if (corruption && inBloc && (stance === "deny" || stance === "reform")) {
    // Skandalı kapatmak: lider veya ortak reform/deny ile üstlenebilir
    if (isLead || inBloc) {
      updateSimulation(ctx.simulationId, { pending_crisis: null });
  ...
  insertEvent(
    ...
    "tool_executed",
    {
      ...
      stance,
      statementText,
      message: `${actor.name} PR (${stance}): ${statementText}${resignNote}`,
    },
    sim.month
  );
  return ok(`PR: ${stance}${resignNote}`);
}
...
function moveConfidence(
  ...
  if (!["censure", "confidence"].includes(motionType)) {
    return fail("motionType censure|confidence olmalı");
  }
  ...
  if (needsCabinetFormation(ctx.simulationId)) {
    return fail("Kabine kurulmadan gensoru/güvenoyu açılamaz");
  }

  const sim = getSimulation(ctx.simulationId)!;
  const actor = getParty(ctx.actorPartyId)!;
  const gov = getParties(ctx.simulationId).find((p) => p.is_government);
  if (!gov) return fail("Mühürlü iktidar yok — gensoru hedefi bulunamadı");

  const inBloc = isGovernmentBlocMember(ctx.simulationId, ctx.actorPartyId);

  if (motionType === "censure") {
    if (inBloc) {
      return fail(
        "Gensoru yalnız muhalefet verir. İktidar/koalisyon ortağı confidence kullanabilir."
      );
    }
  } else {
    // confidence: yalnız iktidar lideri
    if (!actor.is_government) {
      return fail(
        "Güvenoyu yalnız mühürlü iktidar lideri açabilir. Muhalefet censure (gensoru) kullanır."
      );
    }
  }
  ...
  insertEvent(
    ...
    "confidence_motion",
    {
      message: `${actor.name} ${motionType === "censure" ? "gensoru" : "güvenoyu"} verdi: ${rationale}`,
      partyName: actor.name,
      partyColor: actor.color,
      motionId: id,
      motionType,
  ...
  return ok(
    `${motionType === "censure" ? "Gensoru" : "Güvenoyu"} başlatıldı`,
    { motionId: id }
  );
}
