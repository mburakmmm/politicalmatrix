...
import { currentGovernmentBlocIds } from "./metrics";

function crisisBlamePayload(
  simId: string,
  crisis: string,
  message: string,
  extra: Record<string, unknown> = {}
): Record<string, unknown> {
  const bloc = currentGovernmentBlocIds(simId);
  return {
    crisis,
    message,
    blameLeadId: bloc.leadId,
    blamePartyIds: bloc.all,
    ...extra,
  };
}
...
export function maybeTriggerCrisis(sim: SimulationRow): CrisisType {
  ...
  if (passed.length && rng() < 0.08) {
    ...
    insertEvent(
      sim.id,
      "crisis",
      crisisBlamePayload(sim.id, "aym_veto", `Anayasa Mahkemesi iptali: "${bill.title}" yürürlükten kalktı. İktidar yargı reformu ile yanıt verebilir.`, {
        billId: bill.id,
