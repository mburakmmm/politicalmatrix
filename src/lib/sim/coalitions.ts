  ...
  if (hasMajority(total)) {
    if (!sealed) {
      ...
      for (const partnerId of partners) {
        // Kota sandalye oranına göre (minCount zorlama yok)
        shareMinistriesForAlliance(sim.id, leadId, partnerId);
      }
    } else if (
      sim.phase !== "governing" &&
      sim.phase !== "voting" &&
...
export function describeAlliances(simulationId: string): string {
  ...
  return alliances
    .map((a) => {
      const stressNote =
        a.status === "accepted" && (a.stress ?? 0) > 0
          ? ` stress=${(a.stress ?? 0).toFixed(0)}`
          : "";
      return `${name(a.from_party_id)} → ${name(a.to_party_id)} [${a.status}${stressNote}]: ${a.concessions}`;
    })
    .join("\n");
}
