...
export function lawFitsIdeology(
  ...
  // Merkez: gri alan (0) ve karşıt (-1/-2) yasak — yalnızca net uyum (>=1)
  const minScore = key === "center" ? 1 : 0;
  if (score < minScore) {
    return {
      ok: false,
      score,
      reason:
        key === "center"
          ? `“${law.title}” (${law.id}) Merkez çizgisine yeterince yakın değil (skor ${score}, min 1). Uç veya gri alan teklifleri kapalı.`
          : `“${law.title}” (${law.id}) ideolojinize ters (skor ${score}). Uyumlu bir katalog id seçin.`,
    };
  }
