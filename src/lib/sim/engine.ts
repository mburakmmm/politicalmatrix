...
import { applyGovernmentPollPressure } from "./govPollPressure";
import { applyMinistryMonthlyEffects } from "./ministryEffects";
import {
  runElection,
...
export async function runSimulationTick(
  ...
  try {
    ...
    if (
      ...
      runElection(current, "2 yıllık olağan seçim");
    ...
    if (current.phase !== "election") {
      ...
      applyMinistryMonthlyEffects(current.id);
      applyGovernmentPollPressure(current.id);
