...
export function describeLawsForAgent(
  ...
  const enacted = new Set(states.map((s) => s.law_id));
  const stateLine =
    states.length === 0
  ...
  const suggestions = suggestLawsForSlug(partySlug, 8, enacted);
  ...
  return [
    `Kanun kataloğu: ${stats.total} madde / ${stats.groups} grup. ÖNCELİK: proposeLaw(lawId) — yalnız ideolojine uyumlu (bias≥0).`,
