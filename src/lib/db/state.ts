...
export async function buildSimulationState(): Promise<SimulationState> {
  ...
  const partnerIdSet = new Set(partners);
  ...
  const sealedMonth = sim.gov_sealed_month ?? null;
  const tenure =
    sealedMonth != null && gov
      ? Math.max(0, sim.month - sealedMonth)
      : 99;
  const honeymoon = !!gov && tenure < 8;
  const honeymoonMonthsLeft = honeymoon ? Math.max(0, 8 - tenure) : 0;

  const latestCrisis = [...parsedEvents].reverse().find((e) => e.type === "crisis");
  let crisisInfo: SimulationState["situation"]["crisis"] = null;
  if (sim.pending_crisis || latestCrisis) {
    const payload = latestCrisis?.payload || {};
    const blameIds = Array.isArray(payload.blamePartyIds)
      ? payload.blamePartyIds.map(String)
      : [];
    const blameNames = blameIds
      .map((id) => parties.find((p) => p.id === id)?.name)
      .filter(Boolean) as string[];
    const blameLeadName =
      parties.find((p) => p.id === String(payload.blameLeadId || ""))?.name ||
      blameNames[0] ||
      null;
    crisisInfo = {
      type: String(sim.pending_crisis || payload.crisis || "crisis"),
      blameNames,
      blameLeadName,
    };
  }

  const recentMonthEvents = parsedEvents.filter(
    (e) => e.month >= sim.month - 1
  );
  const idleIds = new Set(
    recentMonthEvents
      .filter((e) => e.type === "party_idle")
      .map((e) => String(e.payload.partyName || ""))
  );
  const rebellionIds = new Set(
    recentMonthEvents
      .filter((e) => e.type === "mp_rebellion")
      .map((e) => String(e.payload.partyName || ""))
  );
  const blameIdSet = new Set(
    Array.isArray(latestCrisis?.payload?.blamePartyIds)
      ? (latestCrisis!.payload.blamePartyIds as unknown[]).map(String)
      : []
  );
  ...
  return {
    ...
    parties: parties.map((p) => {
      const signals: Array<"idle" | "rebellion" | "honeymoon" | "blame"> = [];
      if (idleIds.has(p.name)) signals.push("idle");
      if (rebellionIds.has(p.name)) signals.push("rebellion");
      if (
        honeymoon &&
        (p.is_government || partnerIdSet.has(p.id))
      ) {
        signals.push("honeymoon");
      }
      if (blameIdSet.has(p.id)) {
        signals.push("blame");
      }
      return {
        id: p.id,
        ...
        is_coalition_partner: partnerIdSet.has(p.id) && !p.is_government,
    ...
    alliances: alliances.map((a) => {
      ...
      return {
        ...
        stress: a.stress ?? 0,
        consecutive_nos: a.consecutive_nos ?? 0,
      };
    }),
    ...
    situation: {
      ...
      honeymoon,
      honeymoonMonthsLeft,
      govSealedMonth: sealedMonth,
      crisis: crisisInfo,
      phaseLabel: PHASE_LABELS[sim.phase] || sim.phase,
    },
