...
export interface SimulationRow {
  ...
  mandate_rank?: number;
  /** Anket zayıflığı üst üste ay sayısı (censure bandı) */
  gov_poll_weak_streak?: number;
  /** Kabine mühür ayı — balayı / miras fatura */
  gov_sealed_month?: number | null;
...
export interface AllianceRow {
  ...
  created_month: number;
  /** Koalisyon ideolojik gerilimi 0–100 */
  stress?: number;
  consecutive_nos?: number;
  stress_updated_month?: number | null;
}
...
export interface PartyPublic {
  ...
  /** Kabul edilmiş ittifakta iktidar/formateur ortağı */
  ...
  summary?: string;
  ministries?: string[];
  /** UI sinyalleri: idle / isyan / balayı / kriz suçu */
  signals?: Array<"idle" | "rebellion" | "honeymoon" | "blame">;
}

export interface SituationPublic {
  governmentSeats: number;
  majority: boolean;
  honeymoon: boolean;
  honeymoonMonthsLeft: number;
  govSealedMonth: number | null;
  crisis: {
    type: string;
    blameNames: string[];
    blameLeadName: string | null;
  } | null;
  phaseLabel: string;
}
...
/** Dönem uzunluğu (ay). 24 = 2 yılda bir olağan seçim */
